...
public class PaqueteCotizacionDAO implements IPaqueteCotizacionDAO {
    ...
    @Override
    public List<ProductoCantidad> obtenerProductosPorPaquete(int paqueteId) throws PersistenciaException {

        String sql = """
            SELECT p.id, p.nombre, p.categoria, p.capacidad, p.precio_base, pp.cantidad_base
    ...
    CategoriaProducto.valueOf(rs.getString("categoria").toUpperCase())
            ...
            throw new PersistenciaException("Error al obtener productos del paquete: " + ex.getMessage(), ex);
        }
    }
