...
function wireStep2(){
  ...
  $('#btnNext2').addEventListener('click', () => {
    ...
    const consumoNum = Number(String(consumoRaw).replace(/,/g, '').trim());
        ...
        if (consumoRaw.trim() == '' || isNaN(consumoNum)) {
            msg.textContent = 'El consumo del periodo debe ser un valor númerico.';
            toast({tittle: 'Consumo inválido', message: 'Los consumos son obligatorios y deben ser valores numéricos postivos.', icon:'alert-triangle'});
            return;
        }
        
        if(consumoNum <= 0){
            msg.textContent = 'El consumo del periodo debe ser un valor numérico positivo mayor a cero.';
            toast({tittle:'Consumo inválido', message:'Los consumos son obligatorios y deben ser valores numéricos positivos.', icon:"alert-triangle"});
            return;
        }
