# --- Archivo: tarifa_domestica_bimestral.py ---
...
# --- Esta es la función principal que importará app_maestro ---
def procesar_tarifa_domestica_bimestral():
    ...
    pdf_path = filedialog.askopenfilename(title="Selecciona el recibo CFE (Doméstica Bimestral)", filetypes=[("Archivos PDF", "*.pdf")])
    if not pdf_path:
    ...
    if tarifa_encontrada not in tarifas_validas:
        messagebox.showerror(
            "Tarifa Incorrecta",
            f"¡Error! Seleccionaste 'Doméstica Bimestral', pero este recibo es de tarifa '{tarifa_encontrada}'.\n\n"
    ...
    excluir_palabras = ["TOTAL A PAGAR", "FOTOVOLTAICO", "DESCARGA", "APP"]
    for i, linea in enumerate(lineas):
        ...
        if "NO. DE SERVICIO" in linea:
            break
        if any(palabra in linea.upper() for palabra in excluir_palabras):
            continue
    ...
    for i in range(6):
        h1[f"I{3 + i}"] = consumos[i] if i < len(consumos) else ""
        h1[f"I{19 + i}"] = pagos[i] if i < len(pagos) else ""
    ...
    nuevo_path = os.path.join(r"D:/SECOM/Cotizaciones José", f"COTIZACION SISTEMA FOTOVOLTAICO DOMESTICA BIMESTRAL {nombre_archivo}.xlsm")
