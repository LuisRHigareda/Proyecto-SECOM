...
# (Versión actualizada: Incluye Cálculo de Impacto Ambiental en G21)
...
def procesar_tarifa_domestica_mensual():
    ...
    # --- NUEVA FUNCIÓN: CÁLCULO DE IMPACTO AMBIENTAL ---
    def calcular_impacto_ambiental(energia_anual_kwh):
        """
        Calcula toneladas de CO2 y árboles equivalentes.
        Factor de emisión México: 0.423 kg CO2e/kWh
        Absorción árbol urbano: ~20kg/año
        """
        ...
        return {
            "co2_20anos": round(co2_evitado_ton * 20, 1),
            "arboles_20anos": int(arboles * 20)
        }
    ...
    # --- INTEGRACIÓN CLIMATE TECH (Impacto Ambiental) ---
    # 1. Calculamos energía anual total basada en consumos extraídos
    energia_anual_total = sum(consumos)
    
    # 2. Obtenemos los datos calculados
    datos_eco = calcular_impacto_ambiental(energia_anual_total)
    
    # 3. Texto para el reporte
    texto_impacto = (
        f"PLANETA:\n"
        f"• Evitarás {datos_eco['co2_20anos']} Tons de CO2 en 20 años.\n"
        f"• Equivalente a plantar {datos_eco['arboles_20anos']} árboles 🌳."
    )
    
    # 4. Insertar en Celda G21
    h2["A24"] = texto_impacto
    # ----------------------------------------------------
